...
# bash pred_zh.sh gec_path=exp/bart.0/model.baseline.full_shot.2   dataset=fcgec.dev suffix=baseline.full_shot.lm_gecdi lm_alpha=0.1 lm_beta=1.0
# bash pred_zh.sh gec_path=exp/bart.0/model.baseline.full_shot.2   dataset=fcgec.dev suffix=baseline.full_shot
# bash pred_zh.sh gec_path=exp/bart.0/model.aug.full_shot.3   dataset=fcgec.dev suffix=aug.full_shot
...

{
    ...
    echo "lm_path:   ${lm_path:=models/gpt2-chinese-cluecorpussmall}"
    ...
    if [ "$(echo "$ged_alpha == 0" | bc -l)" -eq 1 ]; then
        if [ "$(echo "$lm_alpha == 0" | bc -l)" -eq 1 ]; then
            echo "pred:      ${pred:=$(dirname "$gec_path")/pred/$dataset/$prefix$suffix/$dataset.beam-"$beam".pred}"
        else
...

    {
        ...
        if [[ $dataset =~ "nlpcc18" ]]; then
            ...
            if [[ $dataset =~ "fcgec.test" ]]; then
                paste "$input" "$pred".out | awk '{print NR"\t"$p}' >"$pred".full.para
                python scripts/fcgec_test_refine.py --input "$pred".full.para --output "$pred".para
            else
            ...
            python compare_m2_for_evaluation.py -v -hyp "$workdir"/"$pred".m2 -ref "$workdir"/"$gold" -cat 1 | tee "$workdir"/"$pred".cherrant.log
            cd "$workdir"
        fi
        rm "$pred".split
        rm "$pred".tmp*
