...
class Seq2SeqModel(Model):
    ...
    def avg_loss(self, x, tgt, tgt_mask, ref_nums, s_y):
        i = 0
        total_loss = 0
        for ref_num in ref_nums:
            avg_loss = 0
            for ref_index in range(ref_num):
                loss = self.criterion(s_y[i: i + 1][tgt_mask[i: i + 1]], tgt[i: i + 1][tgt_mask[i: i + 1]])
                i += 1
                avg_loss += loss
            avg_loss /= ref_num
            total_loss += avg_loss
        assert i == int(x.shape[0])
        return total_loss / len(ref_nums)

    def min_loss(self, x, tgt, tgt_mask, ref_nums, s_y):
        i = 0
        total_loss = 0
        for ref_num in ref_nums:
            min_loss = math.inf
    ...
    def max_loss(self, x, tgt, tgt_mask, ref_nums, s_y):
        i = 0
        total_loss = 0
        for ref_num in ref_nums:
            max_loss = -math.inf
    ...
    def sum_loss(self, x, tgt, tgt_mask, ref_nums, s_y):
        return self.criterion(s_y[tgt_mask], tgt[tgt_mask])

    def mr_loss(self, x, tgt, src_mask, tgt_mask, ref_nums, cur_step):
        assert self.args.encoder == 'bart'
        x = torch.cat([torch.stack([x[i]] * ref_nums[i], 0) for i in range(len(ref_nums))], 0)
        src_mask = torch.cat([torch.stack([src_mask[i]] * ref_nums[i], 0) for i in range(len(ref_nums))], 0)
        shifted = torch.full_like(tgt, self.args.eos_index)
        shifted[:, 1:] = tgt[:, :-1]
        y = self.decoder(input_ids=shifted, attention_mask=tgt_mask, encoder_hidden_states=x, encoder_attention_mask=src_mask)[0]
        tgt_mask[:, 0] = 0
        y = self.decoder_dropout(y)
        try:
            s_y = self.classifier(y)
            ...
            print(ref_nums)
            breakpoint()
        aggs = self.args.aggs.split("-")
        aggs = aggs * 2 if len(aggs) == 1 else aggs
        if cur_step <= self.args.first_stage_steps:
            agg_loss = getattr(self, aggs[0] + "_loss")
            return agg_loss(x, tgt, tgt_mask, ref_nums, s_y)
        else:
            agg_loss = getattr(self, aggs[1] + "_loss")
            return agg_loss(x, tgt, tgt_mask, ref_nums, s_y)

    def mr_loss_ablate(self, x, tgt, src_mask, tgt_mask, ref_nums, epoch):
        assert self.args.encoder == 'bart'
        x = torch.cat([torch.stack([x[i]] * ref_nums[i], 0) for i in range(len(ref_nums))], 0)
        src_mask = torch.cat([torch.stack([src_mask[i]] * ref_nums[i], 0) for i in range(len(ref_nums))], 0)
        shifted = torch.full_like(tgt, self.args.eos_index)
        ...
        total_loss = 0
        min_indexs = []
        if epoch < self.args.start_min_loss_epoch:
            for ref_num in ref_nums:
                avg_loss = 0
                for ref_index in range(ref_num):
                    loss = self.criterion(s_y[i: i + 1][tgt_mask[i: i + 1]], tgt[i: i + 1][tgt_mask[i: i + 1]])
        ...
        for ref_num in ref_nums:
            ...
            total_loss += min_loss
        assert i == int(x.shape[0])
        return total_loss / len(ref_nums), min_indexs
